...
from auth_dash         import AppIDAuthProviderDas
...
app.layout = html.Div( [ html.Div( id = "page-content" ), dcc.Interval( id = "auth-check-interval", interval = 3600 * 1000 ) ] )
...
@app.callback( Output( "page-content", "children" ), Input( "auth-check-interval", "n_intervals" ) )
@auth.check
def layout_components( n ):   # For example, the following function returns Dropdown and Div UI components that display information about the Flask and Dash frameworks.
  return [
           dcc.Dropdown(
...
@app.callback( Output( 'framework_details', 'children' ), Input( 'frameworks_dropdown', 'value' ) )
